...
from mrmp.utils import timeit, make_hpolytope, draw_cuboid, draw_parallelpiped
...
class Node:
    ...
    def update_plans(
        self, parent:Node, stgcs:STGCS, idx:int, robot_radius:float, tmin:float, tmax:float, 
        starts:List[np.ndarray], goals:List[np.ndarray], t0s:List[float], scaler_multiplier:float, debug:bool=False
    ) -> bool:
        ...
        for j in nx.topological_sort(self.dg.subgraph(replan_list)):
            ...
            if not replan:
                for i in high_priority_agents: # check higher priority agents for conflicts
                    if collision_checking(parent.sols[i].trajectory, parent.sols[j].trajectory, robot_radius, tmin, tmax):
                        replan = True
                        break
...
def PBS(
    ...
    while Stack != []:
        parent, node, j = Stack.pop()
        print(f"-> PBS: Update plan for child {node}.")
        if j is not None and not node.update_plans(parent, stgcs, j, env.robot_radius, T_MIN, tmax, starts, goals, t0s, scaler_multiplier):
            continue
...
def collision_checking_1d(
    ...
    _a_t0 = [np.concatenate([pi_a[0][0:1], [tmin], pi_a[0][:2]])]
    _a_tf = [np.concatenate([pi_a[-1][-2:], pi_a[-1][-2:-1], [tmax]])]
    _b_t0 = [np.concatenate([pi_b[0][0:1], [tmin], pi_b[0][:2]])]
    _b_tf = [np.concatenate([pi_b[-1][-2:], pi_b[-1][-2:-1], [tmax]])]
...
def min_dist_squared(
    ...
    if itvl is None:
        return np.inf # no intersection in time
